...
def visualize2DSoftmax(X, y, model, title=None):
    """
    Visualizes 2D classification decision boundaries using softmax probabilities.

    Creates a contour plot showing the decision boundary learned by a classification model
    for 2D data. Essential for understanding how neural networks learn to separate classes
    in 2D space. Used in L01 to visualize spiral classification and decision boundaries.

    Args:
        X (numpy.ndarray): 2D input data with shape (n_samples, 2).
        y (numpy.ndarray): Class labels for the data points.
        model (torch.nn.Module): Trained classification model.
        title (str, optional): Title for the plot. Defaults to None.

    Returns:
        None: Displays the decision boundary plot with data points overlaid.

    Example:
        Visualize spiral classification (L01_3_Classification_2D):

        ```python
        from introdl.idlmam import visualize2DSoftmax
        import numpy as np

        # Load trained spiral classifier
        model = load_model(SpiralClassifier, MODELS_PATH / 'L01_spiral_classifier.pt')

        # Convert dataset to numpy for visualization
        X_train = train_dataset.tensors[0].numpy()
        y_train = train_dataset.tensors[1].numpy()

        # Visualize decision boundary
        visualize2DSoftmax(X_train, y_train, model,
                          title="Spiral Classification Decision Boundary")
        ```

        Compare before/after training (L01_3_Classification_2D):
        ```python
        # Show untrained model decision boundary
        untrained_model = SpiralClassifier()
        visualize2DSoftmax(X_test, y_test, untrained_model,
                          title="Before Training")

        # Show trained model decision boundary
        trained_model = load_model(SpiralClassifier, checkpoint_file)
        visualize2DSoftmax(X_test, y_test, trained_model,
                          title="After Training")
        ```

        Analysis with different architectures (L01):
        ```python
        # Compare shallow vs deep network decision boundaries
        shallow_model = load_model(ShallowNet, 'shallow_spiral.pt')
        deep_model = load_model(DeepNet, 'deep_spiral.pt')

        # Visualize both
        visualize2DSoftmax(X_test, y_test, shallow_model, "Shallow Network")
        visualize2DSoftmax(X_test, y_test, deep_model, "Deep Network")
        ```
    """
...
def train_network(model, loss_func, train_loader, val_loader=None, test_loader=None, score_funcs=None,
                  ...
                  save_last=False, resume_last=False, total_epochs=None):
    """
    Train neural networks with advanced features like validation, early stopping, and schedulers.

    Comprehensive training function supporting modern deep learning techniques. Includes
    validation monitoring, early stopping, learning rate schedules, gradient clipping,
    and checkpoint management. Used in L02-L06 for progressively complex training scenarios.

    Args:
        model (torch.nn.Module): The neural network model to train.
        loss_func (torch.nn.Module): The loss function to optimize.
        ...
        score_funcs (dict, optional): Evaluation functions {name: function}. Defaults to None.
        epochs (int): Number of training epochs. Defaults to 50.
        device (str): Device for training. Defaults to "cpu".
        checkpoint_file (str, optional): Path to save checkpoints. Defaults to None.
        lr_schedule (dict, optional): Learning rate schedule config. Defaults to None.
        optimizer (torch.optim.Optimizer, optional): Custom optimizer. Defaults to Adam.
        disable_tqdm (bool): Disable progress bar. Defaults to False.
        resume_file (str, optional): Checkpoint file to resume from. Defaults to None.
        resume_checkpoint (bool): Resume from checkpoint_file. Defaults to False.
        early_stop_metric (str, optional): Metric for early stopping. Defaults to None.
        early_stop_crit (str): Early stop criterion ('min' or 'max'). Defaults to "min".
        patience (int): Early stopping patience. Defaults to 4.
        grad_clip (float, optional): Gradient clipping value. Defaults to None.
        scheduler_step_per_batch (bool): Step scheduler per batch vs epoch. Defaults to False.
        plateau_metric (str): Metric for ReduceLROnPlateau scheduler. Defaults to "loss".
        pretend_train (bool): Simulate training with pretrained model. Defaults to False.
        save_last (bool): Save checkpoint after every epoch (not just best). Defaults to False.
        resume_last (bool): Resume from most recent checkpoint instead of best. Defaults to False.
        total_epochs (int, optional): Train to this total epoch count, overrides epochs. Defaults to None.

    Returns:
        pd.DataFrame: Training results with loss, metrics, and timing info.

    Raises:
        ValueError: If pretend_train=True with resume options.

    Example:
        Basic CNN training (L02_3_MNIST_CNN):
        ...
        results_df = train_network(
            ...
            epochs=20,
            device=device,
            checkpoint_file=MODELS_PATH / 'L02_MNIST_CNN.pt'
        )
        ```

        Advanced training with validation (L03_4_Schedulers_with_CIFAR10):
        ```python
        ...
        results_df = train_network(
            ...
            checkpoint_file=MODELS_PATH / 'L03_CIFAR10_Scheduled.pt'
        )
        ```

        Early stopping and custom optimizer (L03_5_Early_Stopping):
        ```python
        import torch.optim as optim

        # Custom optimizer with early stopping
        ...
        print(f"Stopped at epoch: {len(results_df)}")
        ```
        ...
        results_df = train_network(
            ...
            device=device,
            checkpoint_file=MODELS_PATH / 'L05_FineTuned.pt'
        )
        ```
    """
    ...
    return _actual_train_network(
        ...
        save_last, resume_last, total_epochs
    )
...
def _actual_train_network(model, loss_func, train_loader, val_loader=None, test_loader=None, score_funcs=None,
    ...
    # Determine checkpoint files
    best_checkpoint = checkpoint_file
    last_checkpoint = None
    if checkpoint_file:
        last_checkpoint = str(checkpoint_file).replace('.pt', '_last.pt')

    # Handle resume logic with clear priority and warnings
    actual_resume_file = None

    # Check for conflicting parameters
    if resume_last and resume_checkpoint:
        print("Warning: Both resume_last and resume_checkpoint are True. Using resume_last (most recent checkpoint).")

    if resume_last and last_checkpoint and os.path.exists(last_checkpoint):
        actual_resume_file = last_checkpoint
        print(f"Resuming from last checkpoint: {os.path.basename(last_checkpoint)}")
    elif resume_checkpoint and resume_file is None and checkpoint_file and os.path.exists(checkpoint_file):
        actual_resume_file = checkpoint_file
        print(f"Resuming from best checkpoint: {os.path.basename(checkpoint_file)}")
    elif resume_file is not None:
        actual_resume_file = resume_file
        print(f"Resuming from specified file: {os.path.basename(resume_file)}")

    if actual_resume_file is not None:
        start_epoch, total_train_time, results = load_checkpoint(model, optimizer, lr_schedule, actual_resume_file, device)
        print(f"Resuming from epoch {start_epoch}")

    # Calculate epochs to run based on total_epochs
    if total_epochs is not None:
        epochs_to_run = max(0, total_epochs - start_epoch)
        if epochs_to_run == 0:
            print(f"Training already complete for {total_epochs} epochs.")
            print(f"Current epoch: {start_epoch}. Increase total_epochs to continue training.")
    ...
    # Initialize best_epoch - if resuming, find it from loaded results
    best_epoch = start_epoch
    if start_epoch > 0 and results:
        # Find the best epoch from loaded results
        if early_stop_metric:
            metric_key = f"val {early_stop_metric}" if early_stop_metric != "loss" else "val loss"
            if metric_key in results and len(results[metric_key]) > 0:
                if early_stop_crit == "min":
                    best_idx = results[metric_key].index(min(results[metric_key]))
                else:
                    best_idx = results[metric_key].index(max(results[metric_key]))
                best_epoch = results["epoch"][best_idx] if "epoch" in results else best_idx
                best_metric = results[metric_key][best_idx]
        elif val_loader and "val loss" in results and len(results["val loss"]) > 0:
            best_idx = results["val loss"].index(min(results["val loss"]))
            best_epoch = results["epoch"][best_idx] if "epoch" in results else best_idx
        elif test_loader and "test loss" in results and len(results["test loss"]) > 0:
            best_idx = results["test loss"].index(min(results["test loss"]))
            best_epoch = results["epoch"][best_idx] if "epoch" in results else best_idx

    # For progress bar, show total epochs if using total_epochs
    if total_epochs is not None:
        pbar_total = total_epochs
        pbar_initial = start_epoch
        pbar_desc = f"Epoch"
    else:
        pbar_total = epochs_to_run
        pbar_initial = 0
        pbar_desc = "Epoch"

    with tqdm(total=pbar_total, initial=pbar_initial, desc=pbar_desc, disable=disable_tqdm, leave=True, dynamic_ncols=True) as pbar:
        for epoch in range(start_epoch, start_epoch + epochs_to_run):
            ...
            # Update postfix to show current epoch number
            postfix_dict = {'epoch': epoch + 1, 'train_loss': results["train loss"][-1]}
            ...
            if val_loader:
                ...
                with torch.no_grad():
                    ...
                    pbar.set_postfix({'epoch': epoch + 1, 'train_loss': results["train loss"][-1], 'val_loss': results["val loss"][-1]})
            ...
            if test_loader:
                ...
                with torch.no_grad():
                    ...
                    if val_loader:  
                        ...
                        pbar.set_postfix({'epoch': epoch + 1, 'train_loss': results["train loss"][-1], 'test_loss': results["test loss"][-1]})

            # Save best checkpoint (existing behavior)
            # When not using early stopping, we track best based on val/test loss
            if checkpoint_file and early_stop_metric is None:
                save_checkpoint(epoch, model, optimizer, results, checkpoint_file, lr_schedule)
                # Track best epoch based on validation or test loss
                if val_loader and len(results["val loss"]) > 0:
                    if results["val loss"][-1] <= min(results["val loss"]):
                        best_epoch = epoch
                elif test_loader and len(results["test loss"]) > 0:
                    if results["test loss"][-1] <= min(results["test loss"]):
                        best_epoch = epoch

            # Save last checkpoint if requested
            if save_last and last_checkpoint:
                save_checkpoint(epoch, model, optimizer, results, last_checkpoint, lr_schedule)
    ...
    # Report which epoch had the best model if we saved checkpoints
    if checkpoint_file and len(results["epoch"]) > 0:
        final_epoch = results["epoch"][-1]
        if early_stop_metric:
            metric_name = early_stop_metric if early_stop_metric == "loss" else early_stop_metric
            if "val" in metric_name or val_loader:
                print(f"\nBest model saved at epoch {best_epoch + 1} (val {metric_name}: {best_metric:.4f})")
            else:
                print(f"\nBest model saved at epoch {best_epoch + 1} ({metric_name}: {best_metric:.4f})")
        elif val_loader and "val loss" in results:
            best_val_loss = min(results["val loss"])
            print(f"\nBest model saved at epoch {best_epoch + 1} (val loss: {best_val_loss:.4f})")
        elif test_loader and "test loss" in results:
            best_test_loss = min(results["test loss"])
            print(f"\nBest model saved at epoch {best_epoch + 1} (test loss: {best_test_loss:.4f})")
...
def train_simple_network(model, loss_func, train_loader, test_loader=None, score_funcs=None, 
    ...
    Simplified training function using SGD optimizer, designed for introductory lessons.
    Provides basic training loop with progress tracking and checkpoint saving.
    Used in L01 to introduce deep learning training concepts.

    Args:
        model (torch.nn.Module): The neural network model to train.
        loss_func (torch.nn.Module): The loss function to optimize during training.
        train_loader (torch.utils.data.DataLoader): The data loader for the training dataset.
        test_loader (torch.utils.data.DataLoader, optional): The data loader for testing. Defaults to None.
        score_funcs (dict, optional): Dict of evaluation functions {name: function}. Defaults to None.
        ...
        checkpoint_file (str, optional): Path to save model checkpoint. Defaults to None.
        lr (float, optional): The learning rate for SGD optimizer. Defaults to 0.001.
        use_tqdm (bool, optional): Whether to display progress bar. Defaults to True.

    Returns:
        pd.DataFrame: Training results with columns for epoch, loss, and metrics.

    Example:
        Basic neural network training (L01_2_Nonlinear_Regression_1D):
        ...
        # Analyze results
        print(f"Final train loss: {results_df['train loss'].iloc[-1]:.3f}")
        print(f"Final test MAE: {results_df['test MAE'].iloc[-1]:.3f}")
        ```

        Simple classification (L01_3_Classification_2D):
        ```python
        # Train 2D spiral classifier
        ...
        # Visualize decision boundary
        visualize2DSoftmax(model, train_loader.dataset, device)
        ```

    Note:
        This function uses SGD optimizer by default and provides basic training
        functionality for introductory lessons. For more advanced training
        options, use train_network() instead.
    """
...
def _scheduler_accepts_epoch(scheduler):
    """
    Checks if the passed-in scheduler step method accepts an epoch argument.

    Args:
        scheduler: An instance of a PyTorch learning rate scheduler.

    Returns:
        bool: True if the scheduler step method accepts an epoch, False otherwise.
    """
    # Schedulers that accept the `epoch` argument in their step method
    schedulers_with_epoch_arg = (
        # torch.optim.lr_scheduler.StepLR,  # Commented out
...
# === REMOVED_FUNCTION_START: test_network ===
...
# === REMOVED_FUNCTION_END: test_network ===


# === REMOVED_FUNCTION_START: set_seed ===
# def set_seed(seed):
#     torch.manual_seed(seed)
#     np.random.seed(seed)
# === REMOVED_FUNCTION_END: set_seed ===


# === REMOVED_FUNCTION_START: Flatten ===
# class Flatten(nn.Module):
#     def forward(self, input):
#         return input.view(input.size(0), -1)
# === REMOVED_FUNCTION_END: Flatten ===

    
# === REMOVED_FUNCTION_START: View ===
# class View(nn.Module):
#     def __init__(self, *shape):
#         super(View, self).__init__()
#         self.shape = shape
# # === REMOVED_FUNCTION_START: forward ===
# #     def forward(self, input):
# === REMOVED_FUNCTION_END: View ===

#         return input.view(*self.shape) 
# === REMOVED_FUNCTION_END: forward ===

    
# === REMOVED_FUNCTION_START: LambdaLayer ===
...
# # === REMOVED_FUNCTION_START: forward ===
# #     def forward(self, x):
# === REMOVED_FUNCTION_END: LambdaLayer ===

#         return self.lambd(x)
# === REMOVED_FUNCTION_END: forward ===

    
# === REMOVED_FUNCTION_START: DebugShape ===
...
# #     def forward(self, input):
# #         print(input.shape)
# === REMOVED_FUNCTION_END: DebugShape ===

#         return input
# === REMOVED_FUNCTION_END: forward ===

    
# === REMOVED_FUNCTION_START: weight_reset ===
# def weight_reset(m):
#     """
...
#         m.reset_parameters()
#     return
# === REMOVED_FUNCTION_END: weight_reset ===
...
# === REMOVED_FUNCTION_START: LastTimeStep ===
...
# === REMOVED_FUNCTION_END: LastTimeStep ===

#         return last_step.reshape(batch_size, -1)
# === REMOVED_FUNCTION_END: forward ===

    
# === REMOVED_FUNCTION_START: EmbeddingPackable ===
...
# === REMOVED_FUNCTION_END: EmbeddingPackable ===
...
# === REMOVED_FUNCTION_START: ApplyAttention ===
...
# === REMOVED_FUNCTION_END: ApplyAttention ===

#         return final_context, weights
# === REMOVED_FUNCTION_END: forward ===


# === REMOVED_FUNCTION_START: AttentionAvg ===
...
# === REMOVED_FUNCTION_END: AttentionAvg ===
...
# === REMOVED_FUNCTION_START: AdditiveAttentionScore ===
...
# === REMOVED_FUNCTION_END: AdditiveAttentionScore ===

#         return scores
# === REMOVED_FUNCTION_END: forward ===


# === REMOVED_FUNCTION_START: GeneralScore ===
...
# === REMOVED_FUNCTION_END: GeneralScore ===

#         return scores
# === REMOVED_FUNCTION_END: forward ===


# === REMOVED_FUNCTION_START: DotScore ===
...
# === REMOVED_FUNCTION_END: DotScore ===

#         return scores
# === REMOVED_FUNCTION_END: forward ===

    
# === REMOVED_FUNCTION_START: getMaskByFill ===
...
#     return mask
# === REMOVED_FUNCTION_END: getMaskByFill ===


# === REMOVED_FUNCTION_START: LanguageNameDataset ===
...
# === REMOVED_FUNCTION_END: LanguageNameDataset ===

    
# === REMOVED_FUNCTION_START: pad_and_pack ===
...
# === REMOVED_FUNCTION_END: pad_and_pack ===
